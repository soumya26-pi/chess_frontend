...
function handleSquareClick(event) {
    const squareLabel = event.target.dataset.label;
    displayNotification(`Square clicked: ${squareLabel}`);
...
function loadPiece(piece, position) {
    ...
    // Add data-label attribute to the piece element
    pieceElement.dataset.label = squareElement.dataset.label;

    squareElement.appendChild(pieceElement);
}

function changeSquareColor() {
    const squareNameInput = document.getElementById('squareName');
    const colorSelect = document.getElementById('colorSelect');

    const squareName = squareNameInput.value.trim().toUpperCase();
    const color = colorSelect.value;

    const validSquarePattern = /^[A-H][1-8]$/;

    if (validSquarePattern.test(squareName)) {
        // Convert chess notation to numerical indices
        const columnIndex = squareName.charCodeAt(0) - 'A'.charCodeAt(0);
        const rowIndex = 8 - parseInt(squareName.charAt(1), 10) + 1;

        if (columnIndex >= 0 && columnIndex < 8 && rowIndex >= 1 && rowIndex <= 8) {
            const squareElement = document.getElementById(`${rowIndex}${columnIndex}`);
            
            if (squareElement) {
                squareElement.style.backgroundColor = color;
            } else {
                displayNotification(`Square not found: ${squareName}`);
            }
        } else {
            displayNotification(`Invalid square name: ${squareName}`);
        }
    } else {
        displayNotification(`Invalid square name: ${squareName}`);
    }
...
function movePiece(piece, startingPosition, endingPosition) {
    ...
    displayNotification(`${piece.id.toUpperCase()} moved from ${getSquareLabel(startingPosition)} to ${getSquareLabel(endingPosition)}`);
}

function getSquareLabel(position) {
    const columnLabels = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
    const rowLabel = 8 - position[0];
    const columnLabel = columnLabels[position[1]];
